...
let ig q = App1 (FullGateSet.U_I, q)
let x q = App1 (FullGateSet.U_X, q)
let y q = App1 (FullGateSet.U_Y, q)
let z q = App1 (FullGateSet.U_Z, q)
let h q = App1 (FullGateSet.U_H, q)
let s q = App1 (FullGateSet.U_S, q)
let t q = App1 (FullGateSet.U_T, q)
let sdg q = App1 (FullGateSet.U_Sdg, q)
let tdg q = App1 (FullGateSet.U_Tdg, q)
let rx r q = App1 (FullGateSet.U_Rx(r), q)
let ry r q = App1 (FullGateSet.U_Ry(r), q)
let rz r q = App1 (FullGateSet.U_Rz(r), q)
let rzq r q = App1 (FullGateSet.U_Rzq(r), q)
let u1 r1 q = App1 (FullGateSet.U_U1(r1), q)
let u2 r1 r2 q = App1 (FullGateSet.U_U2(r1,r2), q)
let u3 r1 r2 r3 q = App1 (FullGateSet.U_U3(r1,r2,r3), q)
let cx q1 q2 = App2 (FullGateSet.U_CX, q1, q2)
let cz q1 q2 = App2 (FullGateSet.U_CZ, q1, q2)
let swap q1 q2 = App2 (FullGateSet.U_SWAP, q1, q2)
let ccx q1 q2 q3 = App3 (FullGateSet.U_CCX, q1, q2, q3)
let ccz q1 q2 q3= App3 (FullGateSet.U_CCZ, q1, q2, q3)
...
let sqir_to_qasm_gate oc g =
   match g with
   | App1 (FullGateSet.U_I, n) -> fprintf oc "id q[%d];\n" n
   | App1 (FullGateSet.U_X, n) -> fprintf oc "x q[%d];\n" n
   | App1 (FullGateSet.U_Y, n) -> fprintf oc "y q[%d];\n" n
   | App1 (FullGateSet.U_Z, n) -> fprintf oc "z q[%d];\n" n
   | App1 (FullGateSet.U_H, n) -> fprintf oc "h q[%d];\n" n
   | App1 (FullGateSet.U_S, n) -> fprintf oc "s q[%d];\n" n
   | App1 (FullGateSet.U_T, n) -> fprintf oc "t q[%d];\n" n
   | App1 (FullGateSet.U_Sdg, n) -> fprintf oc "sdg q[%d];\n" n
   | App1 (FullGateSet.U_Tdg, n) -> fprintf oc "tdg q[%d];\n" n
   | App1 (FullGateSet.U_Rx(r), n) -> fprintf oc "rx(%f) q[%d];\n" r n
   | App1 (FullGateSet.U_Ry(r), n) -> fprintf oc "ry(%f) q[%d];\n" r n
   | App1 (FullGateSet.U_Rz(r), n) -> fprintf oc "rz(%f) q[%d];\n" r n
   | App1 (FullGateSet.U_Rzq(q), n) -> 
       fprintf oc "rzq(%a,%a) q[%d];\n" Z.output (Q.num q) Z.output (Q.den q) n
   | App1 (FullGateSet.U_U1(r1), n) -> fprintf oc "u1(%f) q[%d];\n" r1 n
   | App1 (FullGateSet.U_U2(r1,r2), n) -> fprintf oc "u2(%f,%f) q[%d];\n" r1 r2 n
   | App1 (FullGateSet.U_U3(r1,r2,r3), n) -> fprintf oc "u3(%f,%f,%f) q[%d];\n" r1 r2 r3 n
   | App2 (FullGateSet.U_CX, m, n) -> fprintf oc "cx q[%d], q[%d];\n" m n
   | App2 (FullGateSet.U_CZ, m, n) -> fprintf oc "cz q[%d], q[%d];\n" m n
   | App2 (FullGateSet.U_SWAP, m, n) -> fprintf oc "swap q[%d], q[%d];\n" m n
   | App3 (FullGateSet.U_CCX, m, n, p) -> fprintf oc "ccx q[%d], q[%d], q[%d];\n" m n p
   | App3 (FullGateSet.U_CCZ, m, n, p) -> fprintf oc "ccz q[%d], q[%d], q[%d];\n" m n p
